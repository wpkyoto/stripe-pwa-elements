  ...
  describe("method test", () => {
    ...
    describe('#componentWillUpdate', () => {
      ...
      it.each([
        ['' as const],
        ['failure' as const]
      ])("If the publishableKey is not provided, should not call initStripe method(status: %s)", async (loadingStatus) => {
      ...
      it.each([
        ['' as const],
        ['failure' as const]
      ])('Should call initStripe method when the status is not a part of "success" or "loading" (status: %s)', async (loadingStatus) => {
      ...
      it.each([
        ['success' as const],
        ['loading' as const]
      ])('Should not call initStripe method when the status is a part of "success" or "loading" (status: %s)', async (loadingStatus) => {
        stripeStore.set('publishableKey', 'pk_test_xxxx')
        stripeStore.set('loadStripeStatus', loadingStatus)
        element.componentWillUpdate()
         expect(element.initStripe).toHaveBeenCalledTimes(0)
